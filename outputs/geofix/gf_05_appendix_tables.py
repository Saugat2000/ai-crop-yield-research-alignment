...
def main():
    lg = RunLogger("gf_05_appendix_tables")
    ...
    # research counts come from the corrected allocation
    panc = pd.read_parquet(HERE / "country_crop_panel_corrected.parquet")
    keepc = [c for c in e0.columns if c not in ("n_studies_fractional", "n_studies_full",
                                                "studied", "has_any_study", "need")]
    e0 = e0[keepc].merge(panc[["iso3", "crop_standard_name", "n_studies_fractional"]],
                         on=["iso3", "crop_standard_name"], how="left")
    e0["n_studies_fractional"] = e0.n_studies_fractional.fillna(0.0)
    e0["studied"] = (e0.n_studies_fractional > 0).astype(float)
    e = e0.merge(
